...
export class BikeMarketService {
  ...
  getUser(user) {
    this._http.get("/get_user/" + user).subscribe(user => {
  ...
  logout() {
    ...
    this._http
      .delete("/logout")
      .subscribe(response => this.current_user.next(response.json()));
  }
  logged_in(): boolean {
  ...
  createBike(data) {
    this._http.post("/new_bike/" + data.user_id, data).subscribe(bikes => {
  ...
  updateBike(data) {
    this._http
      .post("/update_bike/" + data.bike_id, data.new_bike)
      .subscribe(response => {
  ...
  deleteBike(data) {
    this._http.post("/delete_bike/" + data.bike_id, data).subscribe(bikes => {
      this.retrieveBikes();
      this.retrieveMyBikes(data.user_id);
    });
  }
